...
class VendasController
{
    ...
    public function salvarVendas()
    {
        ...
        if (!empty($erros)) {
            ...
            Redirect::redirecionarComMensagem("/backend/vendas/criar", "error", implode("<br>", $erros));
        }
        ...
        if (!$id_usuario) {
            ...
            Redirect::redirecionarComMensagem("/backend/vendas/criar", "error", "Usuário não autenticado.");
        }
        ...
        if ($id_venda) {
            ...
            Redirect::redirecionarComMensagem("/backend/vendas/listar", "success", "Venda cadastrada com sucesso! ID: {$id_venda}");
        } else {
            error_log("ERRO ao cadastrar venda");
            Redirect::redirecionarComMensagem("/backend/vendas/criar", "error", "Erro ao cadastrar a venda no banco de dados!");
        }
    }
    ...
    public function viewEditarVendas($id_venda)
    {
        ...
        if (!$dados || empty($dados)) {
            ...
            Redirect::redirecionarComMensagem("/backend/vendas/listar", "error", "Venda não encontrada no banco de dados.");
            return;
        }
    ...
    public function atualizarVendas()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            ...
            Redirect::redirecionarComMensagem("/backend/vendas/listar", "error", "Método inválido.");
        }
        ...
        if (!isset($_POST['id_venda']) || empty($_POST['id_venda'])) {
            ...
            Redirect::redirecionarComMensagem("/backend/vendas/listar", "error", "ID da venda não foi informado!");
        }
        ...
        if (!empty($erros)) {
            error_log("Erros de validação: " . implode(", ", $erros));
            Redirect::redirecionarComMensagem("/backend/vendas/editar/{$id_venda}", "error", implode("<br>", $erros));
        }
        ...
        try {
            ...
            if ($resultado) {
                Redirect::redirecionarComMensagem("/backend/vendas/listar", "success", "Venda atualizada com sucesso!");
            } else {
                error_log("ERRO: Update retornou FALSE");
                Redirect::redirecionarComMensagem("/backend/vendas/editar/{$id_venda}", "error", "Erro ao atualizar venda no banco de dados.");
            }
        } catch (\Exception $e) {
            error_log("EXCEÇÃO ao atualizar: " . $e->getMessage());
            Redirect::redirecionarComMensagem("/backend/vendas/editar/{$id_venda}", "error", "Erro: " . $e->getMessage());
        }
    }
    ...
    public function viewExcluirVendas($id_venda)
    {
        ...
        if (!$dados) {
            ...
            Redirect::redirecionarComMensagem("/backend/vendas/listar", "error", "Venda não encontrada.");
        }
    ...
    public function deletarVendas()
    {
        ...
        try {
            ...
            if ($resultado) {
                Redirect::redirecionarComMensagem("/backend/vendas/listar", "success", "Venda excluída com sucesso!");
            } else {
                error_log("ERRO: Delete retornou FALSE");
                Redirect::redirecionarComMensagem("/backend/vendas/listar", "error", "Erro ao excluir venda.");
            }
        } catch (\Exception $e) {
            error_log("EXCEÇÃO ao deletar: " . $e->getMessage());
            Redirect::redirecionarComMensagem("/backend/vendas/listar", "error", "Erro: " . $e->getMessage());
        }
    }
    ...
    public function ativarVenda()
    {
        ...
        if ($this->vendas->ativarVenda($id_venda)) {
            Redirect::redirecionarComMensagem("/backend/vendas/listar", "success", "Venda reativada com sucesso!");
        } else {
            Redirect::redirecionarComMensagem("/backend/vendas/listar", "error", "Erro ao reativar venda.");
        }
    }
