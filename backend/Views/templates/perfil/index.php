...
<a href="/backend/perfil/criar" class="w3-button w3-round-large"
...
<?php if (isset($perfil) && count($perfil) > 0): ?>
  <table border="1" cellpadding="5" cellspacing="0" class="w3-table w3-striped w3-bordered w3-border w3-hoverable">
    <thead>
      <tr class="w3-brown">
    ...
    <tbody>
      <?php foreach ($perfil as $p): ?>
        <tr>
          <td><?= htmlspecialchars($p['id_perfil_usuario']) ?></td>
          <td><?= htmlspecialchars($p['usuario_id']) ?></td>
          <td><?= htmlspecialchars($p['telefone'] ?? 'N/A') ?></td>
          <td><?= htmlspecialchars($p['endereco'] ?? 'N/A') ?></td>
          <td>
            <?php if (!empty($p['foto_perfil_usuario'])): ?>
              <img src="<?= asset_path($p['foto_perfil_usuario']) ?>" alt="Foto"
                style="width:40px;height:40px;border-radius:50%;object-fit:cover;">
            <?php else: ?>
              <i class="fa fa-user-circle w3-xlarge"></i>
            <?php endif; ?>
          </td>
          <td><a href="/backend/perfil/editar/<?= htmlspecialchars($p['id_perfil_usuario']) ?>"
              class="w3-button w3-tiny w3-blue w3-round">Editar</a></td>
          <td><a href="/backend/perfil/excluir/<?= htmlspecialchars($p['id_perfil_usuario']) ?>"
              class="w3-button w3-tiny w3-red w3-round">Excluir</a></td>
        </tr>
      <?php endforeach; ?>
    </tbody>
  </table>
